...
class Scheduler:
    ...
    def handle_results():
        ...
        average_load = (settings.BINOMIAL_P*settings.BINOMIAL_N) / settings.EXPONENTIAL_SCALE  
        for node in NODES:
            ...
            if settings.FOLDER:
                file_nodes.write(",".join([str(node.id), settings.PRECISION.format(settings.EXPONENTIAL_SCALE), settings.PRECISION.format(throughput), settings.PRECISION.format(average_load), settings.PRECISION.format(collision_rate*100), settings.PRECISION.format(loss_rate*100)]))
                file_nodes.write("\n")
        ...
        results_data.append(line)
        if settings.FOLDER:
            file_total.write(",".join([settings.PRECISION.format(settings.EXPONENTIAL_SCALE), settings.PRECISION.format(throughput_total/len(NODES)), settings.PRECISION.format(average_load), settings.PRECISION.format(collision_rate_total/len(NODES)*100), settings.PRECISION.format(loss_rate_total/len(NODES)*100)]))
            file_total.write("\n")
        ...
        if not settings.QUIET:
            ...
            print("\n")
            print(" ".join(["SCALE =", settings.PRECISION.format(settings.EXPONENTIAL_SCALE)]))
            print(" ".join(["RATE =", settings.PRECISION.format(settings.EXPONENTIAL_RATE)]))
            print(" ".join(["SYSTEM THROUGHPUT (kB/s) =", settings.PRECISION.format(throughput_total/1000)]))
            print(" ".join(["AVG OFFERED LOAD (%) =", settings.PRECISION.format(average_load)]))
            print(" ".join(["Packet Size mean =", settings.PRECISION.format((settings.BINOMIAL_P*settings.BINOMIAL_N))]))
            print(" ".join(["I-A Time mean =", settings.PRECISION.format(settings.EXPONENTIAL_SCALE)]))
            print(" ".join(["Actual Throughput saved =", settings.PRECISION.format(throughput_total/len(NODES))]))
