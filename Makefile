...
python_command_run = ./.virtual_env/bin/python run.py
python_command_model = ./.virtual_env/bin/python model/main.py
python_command_prepare = python -m venv --clear .virtual_env && ./.virtual_env/bin/pip3 install -r ./iris/requirements.txt
python_3_command_prepare = python3 -m venv --clear .virtual_env && ./.virtual_env/bin/pip3 install -r ./iris/requirements.txt
...
check_and_run:
	@if [ -d "./.virtual_env" ]; \
		...
		else echo "Please run command make"; \
	fi
	
check_and_model:
	@if [ -d "./.virtual_env" ]; \
		then ${python_command_model}; \
		else echo "Please run command make"; \
	fi
