...
class Node:
    ...
    def generate_next_packet(self):
        ...
        if len(self.queue)-1 < settings.BUFFER:                                                     # since the queue adds each packet as it
            ...
            if settings.VERBOSE:                                                                    # not as it *is* generated, one element
                print(log.success(" ".join([str(self.id), "generated packet", str(packet.id)])))    # the queue is not counted
            # logging
    ...
    def handle_packet(self, packet):                                                                # a new packet, from the point in time
        if self.is_idle():                                                                          # where it *would* have been handled
            ...
            if settings.VERBOSE:                                                                    # sending it queues the next one(s)
            ...
            # logging                                                                               # at the same time by two neighbouring
            if settings.VERBOSE:                                                                    # nodes
                print(log.warning(" ".join([str(self.id), "sent packet", str(packet.id), "(expect a collision)"])))
            # logging
            self._send_packet(packet)
        else:
            # logging
